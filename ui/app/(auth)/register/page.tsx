...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ name: '', phone: '', email: '', password: '', flatId: '' })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const body = { name: form.name, phone: form.phone.trim(), email: form.email || undefined, password: form.password, flatId: form.flatId }
    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(160deg,#050D1A 0%,#0B1628 100%)' }}>
      <div className="glass p-12 text-center max-w-md w-full animate-fade-up">
        ...
        <p className="text-sm leading-relaxed mb-8" style={{ color: '#7B8FAD' }}>
          Your request has been submitted. The society admin will review and approve your account. You can then sign in using your mobile number.
        </p>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12" style={{ background: 'linear-gradient(160deg,#050D1A 0%,#0B1628 100%)' }}>
      <div className="w-full max-w-md animate-fade-up">
        ...
        <p className="text-sm mb-8" style={{ color: '#7B8FAD' }}>First resident of a flat · Requires admin approval</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input className="lux-input" required placeholder="Ravi Kumar"
              value={form.name} onChange={e => set('name', e.target.value)} />
          </div>

          <div>
            <label className="lux-label">Mobile Number <span style={{ color: '#C9A84C' }}>*</span></label>
            <div className="relative">
              <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#4A5E7A' }} />
              <input type="tel" className="lux-input pl-10" required placeholder="9876543210"
                value={form.phone} onChange={e => set('phone', e.target.value)} />
            </div>
            <p className="text-xs mt-1" style={{ color: '#4A5E7A' }}>You will use this to sign in</p>
          </div>

          <div>
            <label className="lux-label">Email <span style={{ color: '#4A5E7A', fontSize: '0.7rem' }}>(optional)</span></label>
            <input type="email" className="lux-input" placeholder="ravi@email.com"
          ...
          <div>
            ...
            <input type="password" className="lux-input" required minLength={8} placeholder="Minimum 8 characters"
